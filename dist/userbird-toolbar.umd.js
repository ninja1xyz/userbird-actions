(function(o,e){typeof exports=="object"&&typeof module<"u"?e(exports):typeof define=="function"&&define.amd?define(["exports"],e):(o=typeof globalThis<"u"?globalThis:o||self,e(o.UserBirdToolbar={}))})(this,function(o){"use strict";const e=r=>`<svg id="Layer_1" href="${r}" xmlns="http://www.w3.org/2000/svg" version="1.1" height="35" width="120" viewBox="0 0 421.39 96.87" style="margin-right: 10px;">Add commentMore actions
  <defs>
    <style>
      .st0 {
        fill: #fff;
...
</svg>`,c={Domain:"userbird.com",Environment:"staging",Url:"https://www.staging-api.userbird.com",LogoUrl:"https://www.userbird.com/logo.svg",LogoWidth:"24px",LogoHeight:"24px",LogoAlt:"UserBird Logo",Localhost:"http://localhost:5173"};class a{constructor({siteId:t,workspaceId:i}){this.siteId=t,this.workspaceId=i,this.logoSvg=null,this.apiToken=null,this.toolbar=null,this.logoRender=null,this.config=null}async setupImports(){this.config=c,this.logoSvg=e("www."+this.Domain+"/")}getApiTokenFromHash(){return new URLSearchParams(window.location.hash.substring(1)).get("birdAuth")}getApiUrl(){return this.config.Environment==="production"?`https://api.${this.config.Domain}/api/profile/`:`https://staging-api.${this.config.Domain}/api/profile/`}getSiteUrl(){return this.config.Environment==="production"?`https://${this.config.Domain}/`:`https://staging.${this.config.Domain}/`}async fetchProfile(){return(await fetch(this.getApiUrl(),{method:"GET",headers:{"Content-Type":"application/json",Authorization:"Bearer "+this.apiToken}})).ok}createToolbar(){this.toolbar=document.createElement("div"),this.toolbar.id="userbird-toolbar",Object.assign(this.toolbar.style,{position:"fixed",bottom:"20px",right:"20px",background:"#222",color:"#fff",padding:"10px 20px",borderRadius:"6px",boxShadow:"0 2px 8px rgba(0,0,0,0.15)",zIndex:"9999",cursor:"move"});const t=`${this.config.Localhost+"/"||this.getSiteUrl()}${this.workspaceId}/site/${this.siteId}`;this.toolbar.innerHTML=`
...
`).join("")}
    ...
    `,document.body.appendChild(this.toolbar),this.makeDraggable()}makeDraggable(){this.toolbar.onmousedown=t=>{t.preventDefault();const i=t.clientX-this.toolbar.getBoundingClientRect().left,s=t.clientY-this.toolbar.getBoundingClientRect().top,h=(n,d)=>{this.toolbar.style.left=n-i+"px",this.toolbar.style.top=d-s+"px",this.toolbar.style.right="auto",this.toolbar.style.bottom="auto",this.toolbar.style.position="fixed"},l=n=>h(n.clientX,n.clientY);document.addEventListener("mousemove",l),document.onmouseup=()=>{document.removeEventListener("mousemove",l),document.onmouseup=null}},this.toolbar.ondragstart=()=>!1}bindEvent(t,i){t.addEventListener("message",s=>{s.origin==="http://localhost:5173"&&(this.apiToken=JSON.parse(s.data).token,console.log(s.data),i(this.apiToken),s.source.postMessage({type:"RESPONSE",text:"Authenticared!"},s.origin))})}async init(){this.setupImports().then(async()=>{this.apiToken=this.apiToken||this.getApiTokenFromHash(),console.log(this.apiToken),this.apiToken&&this.siteId&&this.workspaceId&&await this.fetchProfile()?this.createToolbar():console.warn("UserBird Redirection Unauthenticated.")})}}o.UserBirdToolbar=a,o.default=a,Object.defineProperties(o,{__esModule:{value:!0},[Symbol.toStringTag]:{value:"Module"}})});
