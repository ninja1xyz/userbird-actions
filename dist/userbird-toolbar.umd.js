...
</svg>`,l={Domain:"userbird.com",Environment:"staging",Url:"https://www.staging-api.userbird.com",LogoUrl:"https://www.userbird.com/logo.svg",LogoWidth:"24px",LogoHeight:"24px",LogoAlt:"UserBird Logo",LocalHost:"http://localhost:5173"};class n{constructor({siteId:t,workspaceId:i}){this.siteId=t,this.workspaceId=i,this.logoSvg=null,this.apiToken=null,this.toolbar=null,this.logoRender=null,this.config=null}async setupImports(){this.config=l,this.logoSvg=e("www."+this.Domain+"/")}getApiTokenFromHash(){return new URLSearchParams(window.location.hash.substring(1)).get("birdAuth")}getApiUrl(){return this.config.Environment==="production"?`https://api.${this.config.Domain}/api/profile/`:`https://staging-api.${this.config.Domain}/api/profile/`}getSiteUrl(){return this.config.environment==="production"?`https://${this.config.Domain}/api/profile/`:`https://staging.${this.config.Domain}/api/profile/`}async fetchProfile(){return(await fetch(this.getApiUrl(),{method:"GET",headers:{"Content-Type":"application/json",Authorization:"Bearer "+this.apiToken}})).ok}createToolbar(){this.toolbar=document.createElement("div"),this.toolbar.id="userbird-toolbar",Object.assign(this.toolbar.style,{position:"fixed",bottom:"20px",right:"20px",background:"#222",color:"#fff",padding:"10px 20px",borderRadius:"6px",boxShadow:"0 2px 8px rgba(0,0,0,0.15)",zIndex:"9999",cursor:"move"});const t=`${this.getSiteUrl()||this.config.Localhost}/${this.workspaceId}/site/${this.siteId}`;this.toolbar.innerHTML=`
...
`).join("")}
    ...
    `,document.body.appendChild(this.toolbar),this.makeDraggable()}makeDraggable(){this.toolbar.onmousedown=t=>{t.preventDefault();const i=t.clientX-this.toolbar.getBoundingClientRect().left,c=t.clientY-this.toolbar.getBoundingClientRect().top,h=(s,d)=>{this.toolbar.style.left=s-i+"px",this.toolbar.style.top=d-c+"px",this.toolbar.style.right="auto",this.toolbar.style.bottom="auto",this.toolbar.style.position="fixed"},a=s=>h(s.clientX,s.clientY);document.addEventListener("mousemove",a),document.onmouseup=()=>{document.removeEventListener("mousemove",a),document.onmouseup=null}},this.toolbar.ondragstart=()=>!1}async init(){this.setupImports().then(async()=>{this.apiToken=this.getApiTokenFromHash(),console.log(this.apiToken),this.apiToken&&this.siteId&&this.workspaceId&&await this.fetchProfile()?this.createToolbar():console.warn("UserBird Redirection Unauthenticated.")})}}o.UserBirdToolbar=n,o.default=n,Object.defineProperties(o,{__esModule:{value:!0},[Symbol.toStringTag]:{value:"Module"}})});
